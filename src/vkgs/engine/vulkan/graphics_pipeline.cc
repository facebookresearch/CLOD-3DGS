...
/**
 * @brief Implementation of Vulkan GraphicsPipeline
 */
class GraphicsPipeline::Impl {
 public:
  ...
  Impl(Context context, const GraphicsPipelineCreateInfo& create_info)
    ...
    // if specialization info provided
    if (create_info.use_specialization) {
      stages[1].pSpecializationInfo = &create_info.specialization_info;
    }
