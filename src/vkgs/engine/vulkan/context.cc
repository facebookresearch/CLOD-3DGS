...
/**
 * @brief Implementaion of Vulkan Context
 */
class Context::Impl {
 public:
  Impl(bool vr_mode, bool enable_validation) {
    // instance
    ...
    instance_extensions.push_back(
        VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);
    auto instance_extensions_str = str::const_char_to_string(instance_extensions);
    
    // vr_mode
    if (vr_mode) {
      xr_manager_ = std::make_shared<vk::XRManager>();
      auto xr_instance_extensions = xr_manager_->get_instance_extensions();
      instance_extensions_str = str::combine_vectors_of_string(
        instance_extensions_str,
    ...
    for (int i = 0; i < instance_extensions_str.size(); i++) {
      instance_extensions.push_back(instance_extensions_str[i].c_str());
    }
    ...
    if (enable_validation) {
      CreateDebugUtilsMessengerEXT(instance_, &messenger_info, NULL,
                                   &messenger_);
    }
    ...
    multiview_features.pNext = &timeline_semaphore_features;
    ...
    auto device_extensions_str = str::const_char_to_string(device_extensions);
    if (vr_mode) {
      auto xr_device_extensions = xr_manager_->get_device_extensions(instance_);
      device_extensions_str = str::combine_vectors_of_string(
          device_extensions_str, xr_device_extensions);
    }

    // convert std::string to const char*
    device_extensions.clear();
    for (int i = 0; i < device_extensions_str.size(); i++) {
      device_extensions.push_back(device_extensions_str[i].c_str());
    }
    ...
    if (vr_mode) {
      xr_manager_->start_session(instance_, physical_device_, device_,
                                 graphics_queue_family_index_);
      if (xr_manager_->eye_tracker_enabled()) {
        eye_tracker_ = std::make_shared<eye_tracker::EyeTrackerOpenXR>(
            xr_manager_->instance(), xr_manager_->session());
        auto openxr_eye_tracker =
            std::dynamic_pointer_cast<eye_tracker::EyeTrackerOpenXR>(
                eye_tracker_);
        if (openxr_eye_tracker->status() == XR_SUCCESS) {
          enable_eye_tracking_ = true;
        }
      }
    }
  }
  ...
  std::shared_ptr<XRManager> xr_manager() const noexcept { return xr_manager_; }
  ...
  bool enable_eye_tracker() const { return enable_eye_tracking_; };
  std::shared_ptr<eye_tracker::EyeTracker> eye_tracker() const { return eye_tracker_; };

  void updateEyeTracking() {
    if (enable_eye_tracking_) {
      auto xr_eye_tracker = std::dynamic_pointer_cast<eye_tracker::EyeTrackerOpenXR>(eye_tracker_);
      xr_eye_tracker->update(xr_manager_->frame_state());
    }
  }
  
  glm::vec2 getEyePosition(uint32_t view_index, glm::mat4& view_matrix,
                           glm::mat4& projection_matrix) {
    return eye_tracker_->getLastEyePosition(view_index, view_matrix, projection_matrix);
  }
 ...
 private:
  ...
  // eye tracking
  bool enable_eye_tracking_ = false;
  std::shared_ptr<eye_tracker::EyeTracker> eye_tracker_ = nullptr;
...
Context::Context(bool vr_mode, bool enable_validation)
    : impl_(std::make_shared<Impl>(vr_mode, enable_validation)) {}
...
std::shared_ptr<XRManager> Context::xr_manager() const { return impl_->xr_manager(); }
...
bool Context::enable_eye_tracker() const {
  return impl_->enable_eye_tracker();
}

std::shared_ptr<eye_tracker::EyeTracker> Context::eye_tracker() const { return impl_->eye_tracker(); }

void Context::updateEyeTracking() {
  return impl_->updateEyeTracking();
}

glm::vec2 Context::getEyePosition(uint32_t view_index, glm::mat4& view_matrix, glm::mat4& projection_matrix) {
  return impl_->getEyePosition(view_index, view_matrix, projection_matrix);
}
